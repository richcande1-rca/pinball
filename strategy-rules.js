...
(() => {
  ...
  // Reef already has a reliable completion event generated by its three window
  // targets. Neon Palms and Cafe Ocho are observed below from their established
  // physical geometry so this rule layer does not need to alter business physics.
  ...
  function detectNeonPalmsPass(previousX, previousY, previousSpeed) {
    if (state.features.hotels || hotelBusinessesThisBall.has('neon')) return;
    if (ball.ready || gameOver) return;

    const crossed =
      (previousY > 400 && ball.y <= 400) ||
      (previousY < 400 && ball.y >= 400);
    if (!crossed) return;

    const dy = ball.y - previousY;
    if (Math.abs(dy) < 0.0001) return;
    const t = clamp((400 - previousY) / dy, 0, 1);
    const crossingX = previousX + (ball.x - previousX) * t;
    const currentSpeed = Math.hypot(ball.vx, ball.vy);
    if (
      crossingX >= 387 &&
      crossingX <= 421 &&
      Math.max(previousSpeed, currentSpeed) >= 120
    ) {
      markHotelBusiness('neon');
    }
  }

  function detectCafeOchoCapture() {
    if (state.features.hotels || hotelBusinessesThisBall.has('cafe')) return;
    if (ball.ready || gameOver) return;

    const atCafe = Math.hypot(ball.x - 403, ball.y - 466) <= 1.5;
    const held = Math.hypot(ball.vx, ball.vy) <= 1.5;
    if (atCafe && held) markHotelBusiness('cafe');
  }
  ...
  update = function updateWithStrategy(dt) {
    ...
    const previousSpeed = Math.hypot(ball.vx, ball.vy);

    baseUpdateWithStrategy(dt);

    detectNeonPalmsPass(previousX, previousY, previousSpeed);
    detectCafeOchoCapture();
